...
const CategoryFilters = ({ type = 'plantas', categoryQuery, onFilterChange, hasSearch = false }: CategoryFiltersProps) => {
      ...
      {categories.map((cat) => (
        <button
          ...
          className={`px-4 py-1.5 md:px-6 md:py-2 rounded-full cursor-pointer text-xs md:text-sm font-bold transition-all border ${
            hasSearch ? 'bg-white text-gray-500 border-gray-200 hover:border-[#5B823B] hover:text-[#5B823B]'
            : categoryQuery === cat
