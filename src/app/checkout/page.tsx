          ...
          <div className="lg:col-span-7 space-y-12">
              ...
              <div className="bg-white p-8 rounded-2xl space-y-6 shadow-sm border border-planthia-dark/5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <InputField
                    ...
                    error={touchedFields.has('nombre') ? formErrors.nombre : undefined}
                  />
                  <InputField
                    label="Apellido"
                    placeholder="García"
                    value={formData.apellido}
                    onChange={(val) => updateField('apellido', val)}
                    onBlur={() => handleBlur('apellido')}
                    error={touchedFields.has('apellido') ? formErrors.apellido : undefined}
                  />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <InputField
                  ...
                  <InputField
                    ...
                    error={touchedFields.has('telefono') ? formErrors.telefono : undefined}
                  />
                </div>
                <InputField
                  label="Dirección"
                  ...
                  error={touchedFields.has('direccion') ? formErrors.direccion : undefined}
                />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <InputField
                    label="Ciudad"
                    ...
                    error={touchedFields.has('ciudad') ? formErrors.ciudad : undefined}
                  />
                  <InputField
                    label="Código Postal"
                    placeholder="1234"
                    value={formData.codigoPostal}
                    onChange={(val) => updateField('codigoPostal', val)}
                    onBlur={() => handleBlur('codigoPostal')}
                    error={touchedFields.has('codigoPostal') ? formErrors.codigoPostal : undefined}
                  />
                </div>
              </div>
            </section>
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <PaymentOption
                  label="Mercado Pago"
                  description="Paga con saldo o tarjeta local"
                  icon={<img src="/icons/mercadopago.svg" alt="Mercado Pago" className="w-30 h-30 object-contain" />}
                ...
                <PaymentOption
                  ...
                  icon={<img src="/icons/stripe.svg" alt="Stripe" className="w-30 h-30 object-contain" />}
            ...
            <div className="bg-white p-8 rounded-2xl space-y-8 shadow-sm border border-planthia-dark/5">
              ...
              <div className="space-y-4 pt-6 border-t border-planthia-dark/10">
                ...
                <div className="border-t border-planthia-dark/10 pt-6">
                  <label className="text-sm font-semibold text-planthia-dark/70 block mb-2">
                    ¿Tenés un código de descuento?
                  </label>
                  <div className="flex flex-col sm:flex-row gap-3">
                    <input
                      type="text"
                      placeholder="WELCOME10_XXXXXX"
                      value={discountCode}
                      onChange={handleDiscountChange}
                      className="flex-1 px-4 py-3 bg-planthia-cream border-none rounded-xl focus:ring-2 focus:ring-planthia-green outline-none placeholder:text-planthia-dark/30"
                    />
                    <button
                      type="button"
                      onClick={applyDiscount}
                      className="px-4 sm:px-4 py-3 bg-planthia-green text-white rounded-xl font-bold hover:bg-planthia-light-green transition-all cursor-pointer"
                    >
                      Aplicar
                    </button>
                  </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-4 rounded-full bg-planthia-green text-white font-bold text-lg hover:bg-planthia-light-green transition-all shadow-lg disabled:opacity-50 cursor-pointer"
              >
...
function InputField({ label, placeholder, type = "text", value, onChange, onBlur, error }: InputFieldProps) {
  return (
    <div className="flex flex-col gap-2">
      <label className="text-sm font-semibold text-planthia-dark/70 ml-1">
        {label}
      </label>
      <input
        type={type}
...
function PaymentOption({ label, description, icon, selected, onSelect }: PaymentOptionProps) {
  return (
    ...
    >
      ...
      <div className="scale-150 transition-transform">
        {icon}
      </div>
      {/* <span className="text-xs text-planthia-dark/60 mt-1 text-center">{description}</span> */}
    </div>
  );
}